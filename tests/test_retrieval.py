...
# ---------------------------------------------------------------------------
# search() — the tool-calling entry point (no LLM)
# ---------------------------------------------------------------------------

class TestSearchMethod:

    def test_search_returns_context_and_sources(self, mock_embeddings, monkeypatch):
        """search() must return context str and sources list without calling the LLM."""
        # Patch at the point of use (retrieval.py binds the name locally via 'from ... import')
        monkeypatch.setattr("app.services.retrieval.get_embedding", lambda q: [0.95, 0.05, 0.0])

        from app.services.retrieval import RetrievalService
        svc = RetrievalService()
        result = svc.search("what is finance?")
    ...
    def test_search_sources_contain_finance(self, mock_embeddings, monkeypatch):
        monkeypatch.setattr("app.services.retrieval.get_embedding", lambda q: [0.95, 0.05, 0.0])

        from app.services.retrieval import RetrievalService
        svc = RetrievalService()
        result = svc.search("finance question")
        assert any("finance" in s for s in result["sources"])

    def test_search_context_is_non_empty_string(self, mock_embeddings, monkeypatch):
        monkeypatch.setattr("app.services.retrieval.get_embedding", lambda q: [0.95, 0.05, 0.0])

        from app.services.retrieval import RetrievalService
        svc = RetrievalService()
        result = svc.search("test")
        assert len(result["context"].strip()) > 0
